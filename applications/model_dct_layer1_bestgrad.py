...
class Forward_Model(nn.Module):
    ...
    def forward(self, input, multipliers_list = []):
        ...
        #Calculate output for all multipliers separately
            ...
            for j in multipliers_list: #mul_approx_func_arr:#range(len(mul_approx_func_arr)):
                ...
                #x1 = grad_dct1(weight_rounded, input, self.weight_factor[ii], self.size)
                
                x1 = approx_dct1(torch.unsqueeze(weight_rounded, 0), input, self.weight_factor[ii], self.size, j)
    ...
    def target(self, input):
        target = accurate_dct1(self.weight_orig, input, self.size)
        return target

def approx_dct1(x, input, scale, size, mult_approx_index):
                j = mult_approx_index
                kernel_size = 512
                ...
                for i in range(size):   
                    for hindex in [1,2,3,4]:
                        for vindex in [1,2,3,4]:
                            cur = torch.div(temp22[i*16+(hindex-1)*4+vindex-1], q_50)+0.000001
                            temp[i*16+(hindex-1)*4+vindex-1] = mul_wrapper.round_grad(cur)
                return temp


def accurate_dct1(x, input, size):
                j = 0
                scale = 65535
                x_initial = x #torch.Tensor(np.ones((8,8))) #self.weight
                ...
                for i in range(size):   
                    for hindex in [1,2,3,4]:
                        for vindex in [1,2,3,4]:
                            temp[i*16+(hindex-1)*4+vindex-1] = QuantizeGrad.apply(torch.div(temp22[i*16+(hindex-1)*4+vindex-1], q_50)+0.000001)
