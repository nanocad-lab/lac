...
class Forward_Model(nn.Module):
    ...
    def forward(self, input, multipliers_list = []):
        ...
        #Calculate output for all multipliers separately
            output = torch.zeros([len(multipliers_list), 1, self.size,32,32], dtype=self.dtype)
            for j in multipliers_list: #mul_approx_func_arr:#range(len(mul_approx_func_arr)):
                ...
                #x3 = grad_dct3(weight_rounded, input, self.weight_factor[ii], self.size)
                
                x3 = approx_dct3(torch.unsqueeze(weight_rounded, 0), input, self.weight_factor[ii], self.size, j)#to choose manually - self.mult_list[10]
    ...
    def target(self, input):
        ...
        #target_layer3 = accurate_dct3(self.weight_orig, target_layer2, self.size)
        target = approx_dct_full(self.weight_orig, input, 65536., self.size, -1)
        return target

                
def approx_dct3(x, input, scale, size, mult_approx_index):
                j = mult_approx_index
                kernel_size = 512
                ...
                temp = approx_matmul(x_adjusted_transpose, input, 8, j, swap_inputs=True)/scale
                temp2 = approx_matmul(temp, x_adjusted, 8, j, swap_inputs=True)/scale
                    
                temp_out = mul_wrapper.round_grad(temp2)+128
                for i in range(size):     
...
def accurate_dct3(x, input, size):  
                ...
                temp = torch.matmul(x_adjusted_transpose, input)
...
def approx_dct_full(x, input, scale, size, mult_approx_index):
                j = mult_approx_index
                kernel_size = 512
